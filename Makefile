...
passive_sphere_EPS$(EPS)_RUN$(RUN)/passive_sphere.h5: passive_sphere.p | passive_sphere_EPS$(EPS)_RUN$(RUN)
	...
	(cd passive_sphere_EPS$(EPS)_RUN$(RUN) ; rmpcdmd run chemotactic_cell passive_sphere.p passive_sphere.h5 auto 1>out 2>err)
...
active_sphere_EPS$(EPS)_RUN$(RUN)/active_sphere.h5: active_sphere.p | active_sphere_EPS$(EPS)_RUN$(RUN)
	...
	(cd active_sphere_EPS$(EPS)_RUN$(RUN) ; rmpcdmd run chemotactic_cell active_sphere.p active_sphere.h5 auto 1>out 2>err)
...
nanomotor_EPS$(EPS)_RUN$(RUN)/nanomotor.h5: nanomotor.p | nanomotor_EPS$(EPS)_RUN$(RUN)
	...
	(cd nanomotor_EPS$(EPS)_RUN$(RUN) ; rmpcdmd run chemotactic_cell nanomotor.p nanomotor.h5 auto 1>out 2>err)
